...
def generate_dds_opendds_cpp(
    ...
    for dep in deps:
        ...
        assert len(dep_parts) == 2, (
            "The dependency '%s' must contain a double colon" % dep
        )
    ...
    for idl_file in dds_interface_files:
        ...
        output_path = os.path.join(
            output_basepath, os.path.basename(parent_folder), os.path.basename(folder)
        )
        try:
        ...
        if any_missing:
            print(
                "'%s' failed to generate the expected files for '%s/%s'"
                % (idl_pp, pkg_name, msg_name),
                file=sys.stderr,
            )
        ...
        if "/srv/" in idl_file or "/action/" in idl_file:
            ...
            try:
                ...
                raise RuntimeError(
                    "OpenDDS compiler failed to generate the expected service files"
                )

            any_missing = False
            for suffix in [
                "_RequestResponseC.h",
                "_RequestResponseTypeSupportImpl.cpp",
                "_RequestResponseTypeSupportImpl.h",
                "_RequestResponseTypeSupport.idl",
            ]:
                filename = os.path.join(output_path, msg_name + suffix)
                if not os.path.exists(filename):
                    any_missing = True
            ...
            try:
                generated_idl_file = (
                    output_path + "/" + msg_name + "_RequestResponseTypeSupport.idl"
                )
                cmd = [
                    "tao_idl",
                    generated_idl_file,
                ...
                raise RuntimeError(
                    "TAO IDL compiler failed to generate the expected service files"
                )
...
def generate_cpp(arguments_file):
    mapping = {
        "idl__rosidl_typesupport_opendds_cpp.hpp.em": "%s__rosidl_typesupport_opendds_cpp.hpp",
        "idl__dds_opendds__type_support.cpp.em": "dds_opendds/%s__type_support.cpp",
    }
...
def generate_idl(arguments_file):
    mapping = {
        "idl__rosidl_typesupport_opendds_cpp.idl.em": "dds_opendds/%s__RequestResponse.idl"
    }
